...
import { useDashboardStats, useRecentActivity, useActivityTrends } from '@/lib/hooks/useDashboardData';
...
export default function DashboardPage() {
  ...
  const { data: trendData } = useActivityTrends(userRole?.warehouseId);
  ...
  useEffect(() => {
    ...
    const fetchInventoryByCategory = async () => {
      ...
      if (inventorySnapshot.empty) {
        // Fallback to demo inventory for chart
        let demoInventory = demoData.DEMO_INVENTORY.filter(i => i.warehouseId === userRole.warehouseId);
        if (demoInventory.length === 0) {
          demoInventory = demoData.DEMO_INVENTORY.filter(i => i.warehouseId === 'wh-001');
        }
        const demoCategoryTotals: Record<string, number> = {};

        demoInventory.forEach(item => {
          const product = demoData.DEMO_PRODUCTS.find(p => p.id === item.productId);
          const category = product?.category || 'Other';
          // Calculate VALUE for a high-fidelity look
          const value = (item.quantity || 0) * (product?.unitPrice || 0);
          demoCategoryTotals[category] = (demoCategoryTotals[category] || 0) + value;
        });

        const demoCategoryData = Object.entries(demoCategoryTotals).map(([name, value]) => ({
          name,
          value
        }));
        setInventoryData(demoCategoryData);
        return;
      }
      ...
      inventorySnapshot.forEach(doc => {
        ...
        // Resolve Category using the CATEGORY_MAP if it's a numeric ID
        const categoryId = product?.category || 'Other';
        const categoryName = demoData.CATEGORY_MAP[categoryId] || categoryId;

        // Calculate Total Value for the chart
        const value = (data.quantity || 0) * (product?.unitPrice || 0);
